...
import com.nicolasmilliard.socialcats.session.anInstanceIdProvider
...
class SearchPresenterTest {
    ...
    private val loadingState = Model(isConnected = true, loadingState = Loading)
    ...
    fun before() {
        ...
        sessionManager = SessionManager(auth, fakeStore, anInstanceIdProvider)
    ...
    fun `Test default state and not authenticated`() = runTest {
        withTimeout(500) {
            ...
            searchPresenter.models.onEach { logger.info { "Received $it" } }.filter { it == loadingState }.first()
