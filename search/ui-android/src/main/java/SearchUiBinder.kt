...
import com.nicolasmilliard.socialcats.search.presenter.SearchPresenter.Model.LoadingState.UnAuthenticated
...
class SearchUiBinder(
    ...
    override fun bind(model: SearchPresenter.Model, oldModel: SearchPresenter.Model?) {
        Timber.i("Search UI binder bind: $model")

        when (model.loadingState) {
            is Loading -> displayContentChildId(R.id.progress)
            is UnAuthenticated -> displayContentChildId(R.id.no_content)
            is Failed -> {
